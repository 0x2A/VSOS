...
RamDriveDriver::RamDriveDriver(Device& device) :
	Driver(device), m_ramDrive()
{
}

Driver* RamDriveDriver::CreateInstance(Device* device)
{
	RamDriveDriver* driver = new RamDriveDriver(*device);
...
Result RamDriveDriver::Initialize()
{
	...
	//Map space
	const paddr_t address = (paddr_t)m_device.GetResource((uint32_t)SoftwareDevice::ResourceType::Context);
	...
	//Set properties
	m_device.Name = "RamDrive";

	char buffer[64] = { 0 };
	sprintf(buffer, "Ram Drive - Files: %zu", m_ramDrive->FileCount());
	m_device.Description = buffer;

	return Result::Success;
}

Result RamDriveDriver::Read(char* buffer, size_t length, size_t* bytesRead /*= nullptr*/)
{
