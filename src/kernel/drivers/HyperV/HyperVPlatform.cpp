#include "HyperVPlatform.h"
#include <arch\x64\cpuid.h>
#include <Assert.h>
#include "kernel/Kernel.h"

volatile HV_REFERENCE_TSC_PAGE HyperV::TscPage = { 0 };
...
HV_HYPERCALL_RESULT_VALUE HyperV::HvPostMessage(__in HV_CONNECTION_ID ConnectionId, __in HV_MESSAGE_TYPE MessageType, __in UINT32 PayloadSize, __in_ecount(PayloadSize) PCVOID Message)
{
...
HV_HYPERCALL_RESULT_VALUE HyperV::HvSignalEvent(__in HV_CONNECTION_ID ConnectionId, __in UINT16 FlagNumber /*relative to base number for port */)
{
	cpu_flags_t flags = ArchDisableInterrupts();
