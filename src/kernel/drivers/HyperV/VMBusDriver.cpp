...
const std::string VmBusDriver::GetCompatHID()
{
	return "VMBUS";
}

Driver* VmBusDriver::CreateInstance(Device* device)
{
	VmBusDriver* driver = new VmBusDriver(*device);
...
Result VmBusDriver::Read(char* buffer, size_t length, size_t* bytesRead /*= nullptr*/)
{
...
{
	//Printf("VmBusDriver:OnInterrupt\r\n");
...
{	
	//Printf("VmBusDriver::ThreadLoop begin\r\n");
	while (true)
	{
		...
		case vmbus_channel_message_type::CHANNELMSG_VERSION_RESPONSE:
		{
			...
			//Printf("CHANNELMSG_VERSION_RESPONSE\r\n");
			//Let connect proceed
			m_connectEvent.Set();
		}
		break;
		...
		case vmbus_channel_message_type::CHANNELMSG_OFFERCHANNEL:
		{
			...
			/*if (HyperVDevice::GetDeviceName(child->GetHid().c_str(), name))
				Printf("Added HyperV Device: %s\r\n", name);
			else
				Printf("Added HyperV Device: %s\r\n", child->GetHid().c_str());*/
