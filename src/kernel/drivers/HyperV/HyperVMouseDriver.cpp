...
#include <kernel\devices\hv\HyperVDevice.h>
...
const std::string HyperVMouseDriver::GetCompatHID()
{
	return "{CFA8B69E-5B4A-4CC0-B98B-8BA1A1F3F95A}";
}

Driver* HyperVMouseDriver::CreateInstance(Device* device)
{
	Printf("HyperVMouseDrive instance created\r\n");
	HyperVMouseDriver* driver = new HyperVMouseDriver(*device);
...
Result HyperVMouseDriver::Initialize()
{
	...
	//m_channel = new HyperVChannel(INPUTVSC_SEND_RING_BUFFER_SIZE, INPUTVSC_RECV_RING_BUFFER_SIZE, { &HyperVMouseDriver::Callback, this });
...
Result HyperVMouseDriver::Read(char* buffer, size_t length, size_t* bytesRead /*= nullptr*/)
{
